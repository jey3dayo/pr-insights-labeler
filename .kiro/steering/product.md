...
- 大規模ファイル一覧（上位100ファイル）
- 実行時刻（ISO 8601形式、UTC）
- 改善アクション提案とベストプラクティスの提醒を自動生成

### 🌐 多言語対応

- `language` inputと環境変数（LANGUAGE/LANG）から英語・日本語を自動選択
- i18nextでActionsログ/コメント/サマリーを統一翻訳、既存メッセージはすべて両言語で提供
- `scripts/generate-i18n-types.ts`がpostinstallで型定義を再生成し、翻訳キーの安全性を保証

### 🚨 ラベルベースワークフロー制御

- `fail_on_*`入力でサイズ・ファイル数・行数・リスク超過時にワークフロー失敗を強制
- 付与済みラベルと解析結果を突き合わせ、再実行でも冪等な失敗理由を提示
- i18nログを使って失敗メッセージを通知し、レビュー判断を一貫化
...
ワークフロー出力は言語設定に応じて英語・日本語を切り替え。翻訳キーは型検証済みで、CIログ・コメント・サマリーのメッセージが常に同期されます。
