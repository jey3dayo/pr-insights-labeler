...
最終更新日は書かない。git履歴が正確な情報源であり、手書きの日付は必ず腐るため。
...
- 新規ドキュメント追加フロー:
  ...
  3. 利用者向けで日本語版が無い場合はスタブを作らず、docs/README.md の Translation Gaps 表と追跡 issue に記録する（翻訳は任意なので、中身の無いページを置くと索引とリンクだけが増えて読者を空振りさせるため）
